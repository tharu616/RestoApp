    ...
    <div style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
        ...
        <div ref={headerRef} className="glass"
          style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '14px 28px' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'linear-gradient(135deg,#fb923c,#f43f5e)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '28px' }}>
            {statCards.map((card, i) => (
              <motion.div key={card.label} className="glass-card"
                ...
                transition={{ delay: i * 0.08 }} whileHover={{ y: -5, scale: 1.02 }}
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="glass-card" style={{ borderRadius: '20px', padding: '22px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {orders.slice(0, 5).map((order, i) => {
                    ...
                    return (
                      <motion.div key={order.id} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
                        className="glass" style={{ borderRadius: '12px', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                          <p style={{ color: 'white', fontSize: '13px', fontWeight: 500 }}>{order.customer_name || `Customer #${order.customer_id}`}</p>
                          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '11px', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <Clock size={10} /> Order #{order.id} · Rs. {order.total_price || 0}
                          </p>
                        </div>
                        <span style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '20px', background: sc.bg, color: sc.color }}>{order.status || 'Pending'}</span>
                      </motion.div>
                    );
                  })}
                  {orders.length === 0 && <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>No orders yet</p>}
                </div>
              </div>
              ...
              <div className="glass-card" style={{ borderRadius: '20px', padding: '22px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {menuItems.length === 0 && <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>No menu items yet</p>}
                </div>
              </div>
            </motion.div>
          )}
...
function MenuManager() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="glass-card" style={{ borderRadius: '20px', padding: '22px', alignSelf: 'start' }}>
        ...
        <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <motion.button type="submit" whileTap={{ scale: 0.97 }}
            style={{ padding: '12px', borderRadius: '12px', color: 'white', fontWeight: 600, fontSize: '13px', cursor: 'pointer', border: 'none', background: 'linear-gradient(135deg,#fb923c,#f43f5e)', boxShadow: '0 4px 14px rgba(251,146,60,0.3)' }}>
            Add Item
          </motion.button>
        </form>
      </div>
      ...
      {/* Grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: '14px', alignContent: 'start' }}>
        {items.map((item, i) => (
          <motion.div key={item.id} className="glass-card"
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              ...
              <motion.button onClick={() => handleDelete(item.id)} whileTap={{ scale: 0.85 }}
                style={{ width: '26px', height: '26px', borderRadius: '8px', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', flexShrink: 0 }}>
                ✕
              </motion.button>
            </div>
            {item.description && (
