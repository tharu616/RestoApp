    ...
    <div style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 28px' }}>
          ...
          {activeTab === 'menu' && <MenuManager token={localStorage.getItem('accessToken')} onChanged={fetchData} />}
        </div>
      </div>
    </div>
  );
...
function MenuManager({ token, onChanged }) {
  ...
  const resetForm = () => setForm({ name: '', description: '', price: '', category: '', is_available: true });
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/menu/${id}`, { headers });
  ...
  const saveEdit = async (id) => {
    try {
      await api.put(`/menu/${id}`, editForm, { headers });
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <motion.button onClick={() => startEdit(item)} whileTap={{ scale: 0.85 }} style={{ width: '26px', height: '26px', borderRadius: '8px', background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.2)', color: '#60a5fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}><Pencil size={13} /></motion.button>
                    <motion.button onClick={() => handleDelete(item.id)} whileTap={{ scale: 0.85 }} style={{ width: '26px', height: '26px', borderRadius: '8px', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}><Trash2 size={13} /></motion.button>
                  </div>
                </div>
