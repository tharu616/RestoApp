...
export default function ReservationsManager() {
  ...
  const [form,         setForm]         = useState({
    customer_id: '', table_id: '', date_time: '', time_slot: '', guests: '', notes: ''
  });
  ...
  const handleUpdate = async (id) => {
    try {
      await api.patch(`/reservations/${id}`, editData);
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <form onSubmit={handleCreate}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {[
                  { key: 'customer_id', placeholder: 'Customer ID',           type: 'number',        required: true  },
                  ...
                  { key: 'notes',       placeholder: 'Notes (optional)',       type: 'text',          required: false },
              ...
              {isEditing ? (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                    <input type="datetime-local" value={editData.date_time || ''}
                    ...
                    <input type="text" placeholder="Time Slot" value={editData.time_slot || ''}
                      onChange={(e) => setEditData({ ...editData, time_slot: e.target.value })}
                      className="input-glass" style={{ padding: '9px 12px', borderRadius: '10px', fontSize: '13px' }} />
                    <input type="number" placeholder="Guests" value={editData.guests || ''}
                      onChange={(e) => setEditData({ ...editData, guests: e.target.value })}
                      className="input-glass" style={{ padding: '9px 12px', borderRadius: '10px', fontSize: '13px' }} />
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <motion.button onClick={() => handleUpdate(res.id)} whileTap={{ scale: 0.95 }}
                      style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', borderRadius: '10px', color: 'white', fontSize: '12px', fontWeight: 600, cursor: 'pointer', border: 'none', background: 'linear-gradient(135deg,#fb923c,#f43f5e)' }}>
                      <Check size={13} /> Save
                    </motion.button>
                    <motion.button onClick={() => setEditId(null)} whileTap={{ scale: 0.95 }}
                      style={{ padding: '7px 12px', borderRadius: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.4)', fontSize: '12px', cursor: 'pointer' }}>
                      Discard
                    </motion.button>
                  </div>
                </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      ...
                      <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: '12px', marginTop: '3px' }}>
                        Table {res.table_id || res.table_number || '—'} · {res.guests} guests · {res.time_slot || '—'}
                        {res.date_time ? ` · ${new Date(res.date_time).toLocaleDateString()}` : ''}
                      </p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    {res.status !== 'CANCELLED' && (
                      <>
                        ...
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleCancel(res.id)}
