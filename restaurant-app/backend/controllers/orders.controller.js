...
const create = async (req, res) => {
  const { customer_id, table_id, waiter_id, items } = req.body;
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (menu.rows.length > 0) total += menu.rows[0].price * item.quantity;
    }
    const orderRes = await client.query(
      'INSERT INTO orders (customer_id, table_id, waiter_id, total_price) VALUES ($1,$2,$3,$4) RETURNING *',
      [customer_id || req.user.id, table_id || null, waiter_id || null, total]
    );
